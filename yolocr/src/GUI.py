...
def main(config):
    ...
    time_scsht_row = [
        ...
        sg.T("0:00:00", key="time_scsht_lbl"),
    ]
    cb_dimension_row = [
    ...
    layout = [
        ...
        [sg.Button("Save Settings"), sg.Button("Start OCR"), sg.Cancel()],
    ]
    ...
    while True:
        ...
        if win2_active:
            ev2, vals2 = preview.read(timeout=100)
            print(vals2)
        ...
        if event == "Start OCR":
            subprocess.run(
                "vspipe -y YoloCR.py - | ffmpeg -i - -c:v mpeg4 -qscale:v 3 -y Filtered_video.mp4",
                shell=True,
            )
            asyncio.run(yolocr.main("fra", "Filtered_video.mp4"))
            sg.popup("Finished OCR", font=("Helvetica", 12))
        ...
        if values["has_preview"]:
            if not win2_active:
                ...
                win2_active = True
                preview_window = [
                    [sg.Image(key="preview_image"), sg.T("test")],
                ]
                preview = sg.Window("Preview", preview_window)
                continue
                # preview["preview_image"].update(data=bytes())
